...
/**
 * get_big - gets the bigimum value in the array
 * @array: array to get big value from
 * @size: size of the array
 *
 * Return: bigimum value in the array
 */
int get_big(int *array, size_t size)
{
	...
	big = array[0];	/* start from first index */
...
void counting_sort(int *array, size_t size)
{
	...
	for (i = 0; i < size; i++)
		index[array[i]] += 1;    /* set one to the indexs */

	sumindex = malloc(sizeof(int) * (k + 1));
	if (!sumindex)
		return;
	arr_zero_init(sumindex, k + 1);
	sumindex[0] = index[0];   /* make first index equal */
