 ...
 * @upper: upper bound
 */
void print_parse(int *array, const char *text, size_t low, size_t upper)
{
	size_t i;
	char *separator = "";
 ...
 * @upper: ending index of the array passed
 * @mid: midpoint of the split array
 * @copy: array to store sorted values
 */
void join_parse(int *arr, size_t low, size_t upper, size_t mid, int *copy)
{
	size_t hStart = mid + 1, newIndex, lStart = low, i;

	printf("Merging...\n");
	print_parse(arr, "left", low, mid);
	print_parse(arr, "right", mid + 1, upper);

	for (newIndex = low; newIndex <= upper; newIndex++)
	{
		if ((lStart <= mid && arr[lStart] <= arr[hStart]) || hStart > upper)
	...
	print_parse(arr, "Done", low, upper);
}
...
void merge_sort(int *array, size_t size)
{
	...
	replica = malloc(sizeof(int) * size);
	if (!replica)
		return;

	parser(array, 0, size - 1, replica);
	free(replica);
}
